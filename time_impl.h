 ...
 * include  which includes this header.
  */
 ...
 typedef struct  timespec {		/* definition per POSIX.4 */  <timespec>
 	time_t		tv_sec;		/* seconds */
 	long		tv_nsec;	/* and nanoseconds */
 } timespec_t;  <typedef:timespec_t>
 ...
 typedef struct timespec timestruc_t;	/* definition per SVr4 */  <typedef:timestruc_t>
 ...
 #if (!defined(_POSIX_C_SOURCE) && !defined(_XOPEN_SOURCE)) || \
 	(_POSIX_C_SOURCE > 2) || defined(__EXTENSIONS__)
 typedef struct itimerspec {		/* definition per POSIX.4 */  <itimerspec>
 	struct timespec	it_interval;	/* timer period */
 	struct timespec	it_value;	/* timer expiration */
 } itimerspec_t;  <typedef:itimerspec_t>
